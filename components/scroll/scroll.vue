      ...
      <div ref="listWrapper">
        <slot>
          <ul class="list-content">
            <li @click="clickItem($event,item)" class="list-item" v-for="(item, num) in data" :key="num">{{item}}</li>
          </ul>
        </slot>
      </div>
			<!-- 上拉加载更多 -->
      ...
      >
        <div class="pullup-wrapper" v-if="pullUpLoad && data.length">
          <div class="before-trigger" v-if="!isPullUpLoad">
            <span class="color-gray">{{pullUpTxt}}</span>
          </div>
          <div class="after-trigger" v-else>
            <loading title="拼命加载中···" align="horizontal" color="#999"></loading>
          </div>
        </div>
      </slot>
    </div>
    ...
    >
      <div ref="pulldown" class="pulldown-wrapper" :style="pullDownStyle" v-if="pullDownRefresh">
        <div class="before-trigger" v-if="beforePullDown">
          <!-- <bubble :y="bubbleY"></bubble>  放弃这个组件，在uni-app对应的canvas有兼容问题，具体查看uni-app的canvas用法-->
					<p class="color-gray" >{{bubbleY<=0?'下拉推荐':'松手推荐'}}</p>
        </div>
        <div class="after-trigger" v-else>
          <div v-if="isPullingDown" class="loading">
            <loading title="正在刷新 ···" align="horizontal" color="#999"></loading>
          </div>
          <div v-else><span class="color-gray">{{refreshTxt}}</span></div>
        </div>
      </div>
    </slot>
  </div>
